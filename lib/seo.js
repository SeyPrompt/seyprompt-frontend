...
export const PARENT_COMPANY_NAME = "Sey Ventures";
export const PARENT_COMPANY_URL = "https://www.seyventures.com/";
...
export function getPromptShareImage(prompt) {
  const sampleOutputType = prompt?.sampleOutputType || prompt?.sampleOutput?.type || "";
  const image = String(prompt?.image || prompt?.thumbnail || "").trim();

  return sampleOutputType === "image" && image ? image : DEFAULT_OG_IMAGE_URL;
}
...
export function createPromptMetadata(prompt) {
  ...
  return createPageMetadata({
    ...
    image: getPromptShareImage(prompt),
    imageAlt: prompt.title ? `${prompt.title} sample output` : `${SITE_NAME} prompt preview`,
    type: "article",
    keywords: [
      ...(prompt.tags || []),
...
export function organizationSchema() {
  return {
    ...
    sameAs: [
      ...
      "https://www.youtube.com/@seyprompt",
...
export function promptSchema(prompt) {
  ...
  const description = truncateDescription(prompt.description || SITE_DESCRIPTION);
  const image = absoluteUrl(getPromptShareImage(prompt));
